...
import Icon from 'ant-design-vue/es/icon' // 改报错   template or render function not defined

const { Item, SubMenu } = Menu

// 默认菜单图标数组，如果菜单没配置图标，则会设置从该数组随机取一个图标配置
const iconArr = ['homework', 'user', 'form', 'setting', 'message', 'safety', 'bell', 'delete', 'code-o', 'poweroff', 'eye-o', 'hourglass']
  ...
  methods: {
    renderIcon: function (h, icon) {
      return icon === 'none' ? null
        : h(
          Icon, // 改报错   template or render function not defined
          {
            props: { type: icon !== undefined ? icon : iconArr[Math.floor((Math.random() * iconArr.length))] }
          })
    },
    renderMenuItem: function (h, menu, pIndex, index) {
      let isHidden = menu.meta ? menu.meta.isHidden : false
